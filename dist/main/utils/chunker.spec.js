"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
const vitest_1 = require("vitest");
const chunker_1 = require("./chunker");
const len = (str) => {
    return Buffer.from(str).length;
};
const CHUNK_STRING = "zDq8KDAdv4PwF3UOp3mnEyx1xY71CaY4ZJdPTG8HpLHy3bCYs1x3vwPXdUqY75d0LYHL8KhxgrKqBEK531igiQNk1KqUKmMsabNlwcaF5E2gXA79vpwlxvi1wecwmKGVig4mJ0dzEEXKNsLgyQCsjKOpI7Nw2gnGAKKFdHle1SJeuFj9PyAHx9stMvQFpRQhoLRt3iI0uFA..";
const DOUBLE_CHUNK_STRING = `${CHUNK_STRING}${CHUNK_STRING}`;
(0, vitest_1.describe)("chunker", () => {
    (0, vitest_1.it)("should not chunk and return one item", () => {
        const chunked = (0, chunker_1.createChunks)("my-chunks", "hello-world");
        (0, vitest_1.expect)(chunked.length).toBe(1);
    });
    (0, vitest_1.it)("should chunk and return two chunks", async () => {
        const chunked = (0, chunker_1.createChunks)("my-chunks", CHUNK_STRING, 2000);
        const combined = await (0, chunker_1.combineChunks)("my-chunks", (name) => {
            let chunk = chunked.find((chunk) => {
                return chunk.name === name;
            });
            return chunk?.value;
        });
        (0, vitest_1.expect)(len(`my-chunks=${CHUNK_STRING}`)).toBe(3621);
        (0, vitest_1.expect)(chunked.length).toBe(2);
        (0, vitest_1.expect)(combined).toBe(CHUNK_STRING);
    });
    (0, vitest_1.it)("should chunk and return twelve chunks", async () => {
        const chunked = (0, chunker_1.createChunks)("my-chunks", CHUNK_STRING, 320);
        const combined = await (0, chunker_1.combineChunks)("my-chunks", (name) => {
            let chunk = chunked.find((chunk) => {
                return chunk.name === name;
            });
            return chunk?.value;
        });
        (0, vitest_1.expect)(chunked.length).toBe(12);
        (0, vitest_1.expect)(combined).toBe(CHUNK_STRING);
    });
    (0, vitest_1.it)("should chunk and return one hundred and one chunks", async () => {
        const chunked = (0, chunker_1.createChunks)("my-chunks", CHUNK_STRING, 36);
        const combined = await (0, chunker_1.combineChunks)("my-chunks", (name) => {
            let chunk = chunked.find((chunk) => {
                return chunk.name === name;
            });
            return chunk?.value;
        });
        (0, vitest_1.expect)(chunked.length).toBe(101);
        (0, vitest_1.expect)(combined).toBe(CHUNK_STRING);
    });
    (0, vitest_1.it)("should chunk and return correct size chunks", async () => {
        const key = "sb-xdbaubpgcisziicojymj-auth-token";
        const chunked = (0, chunker_1.createChunks)(key, DOUBLE_CHUNK_STRING);
        const combined = await (0, chunker_1.combineChunks)(key, (name) => {
            let chunk = chunked.find((chunk) => {
                return chunk.name === name;
            });
            return chunk?.value;
        });
        chunked.forEach((chunk, i) => {
            (0, vitest_1.expect)(chunk.name).toBe(`${key}.${i}`);
            (0, vitest_1.expect)([3217, 3217, 899]).toContain(len(`${chunk.name}=${chunk.value}`));
        });
        (0, vitest_1.expect)(chunked.length).toBe(3);
        (0, vitest_1.expect)(len(`${key}=${DOUBLE_CHUNK_STRING}`)).toBe(7257);
        (0, vitest_1.expect)(combined).toBe(DOUBLE_CHUNK_STRING);
    });
    (0, vitest_1.it)("should correctly break between unicode boundaries in escaped characters", () => {
        const test = "   ";
        const chunks = (0, chunker_1.createChunks)("key", test, 4);
        (0, vitest_1.expect)(chunks).toEqual([
            {
                name: "key.0",
                value: " ",
            },
            {
                name: "key.1",
                value: " ",
            },
            {
                name: "key.2",
                value: " ",
            },
        ]);
        (0, vitest_1.expect)(chunks.map((char) => char.value).join("")).toEqual(test);
    });
    (0, vitest_1.describe)("should correctly break between unicode boundaries in long unicode", () => {
        (0, vitest_1.it)("should correctly break between unicode boundaries in long unicode (4 bytes)", () => {
            const test = "🤦🏻‍♂️";
            const chunksAtStartBorder = (0, chunker_1.createChunks)("key", test, 12);
            const chunksAtEndBorder = (0, chunker_1.createChunks)("key", test, 17);
            (0, vitest_1.expect)(chunksAtStartBorder).toEqual(chunksAtEndBorder);
            (0, vitest_1.expect)(chunksAtStartBorder).toEqual([
                {
                    name: "key.0",
                    value: "🤦",
                },
                {
                    name: "key.1",
                    value: "🏻",
                },
                {
                    name: "key.2",
                    value: "‍",
                },
                {
                    name: "key.3",
                    value: "♂",
                },
                {
                    name: "key.4",
                    value: "️",
                },
            ]);
            (0, vitest_1.expect)(chunksAtStartBorder.map((char) => char.value).join("")).toEqual(test);
        });
        (0, vitest_1.it)("should correctly break between unicode boundaries in long unicode (5 bytes)", () => {
            const test = "🤦🏻‍♂️";
            const chunksAtStartBorder = (0, chunker_1.createChunks)("key", test, 18);
            const chunksAtEndBorder = (0, chunker_1.createChunks)("key", test, 20);
            (0, vitest_1.expect)(chunksAtStartBorder).toEqual(chunksAtEndBorder);
            (0, vitest_1.expect)(chunksAtStartBorder).toEqual([
                {
                    name: "key.0",
                    value: "🤦",
                },
                {
                    name: "key.1",
                    value: "🏻",
                },
                {
                    name: "key.2",
                    value: "‍♂",
                },
                {
                    name: "key.3",
                    value: "️",
                },
            ]);
            (0, vitest_1.expect)(chunksAtStartBorder.map((char) => char.value).join("")).toEqual(test);
        });
    });
});
//# sourceMappingURL=chunker.spec.js.map