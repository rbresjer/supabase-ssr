...
describe("createStorageFromOptions in browser without cookie methods", () => {
    ...
    it("should access cookies with various uses of getItem, setItem and removeItem", async () => {
        ...
        let newChunkedValue = Array.from({ length: MAX_CHUNK_SIZE + 1 }, () => "x").join("");
        await storage.setItem("storage-key", newChunkedValue);
        await storage.removeItem("non-existent-item");
        expect(document.cookie).toEqual(`random-cookie=random;storage-key.0=${newChunkedValue.substring(0, MAX_CHUNK_SIZE)};storage-key.1=${newChunkedValue.substring(MAX_CHUNK_SIZE)}`);
        ...
        newChunkedValue = Array.from({ length: 2 * MAX_CHUNK_SIZE + 1 }, () => "x").join("");
        await storage.setItem("storage-key", newChunkedValue);
        expect(document.cookie).toEqual(`random-cookie=random;storage-key.0=${newChunkedValue.substring(0, MAX_CHUNK_SIZE)};storage-key.1=${newChunkedValue.substring(MAX_CHUNK_SIZE, 2 * MAX_CHUNK_SIZE)};storage-key.2=${newChunkedValue.substring(2 * MAX_CHUNK_SIZE)}`);
        newChunkedValue = Array.from({ length: MAX_CHUNK_SIZE + 1 }, () => "x").join("");
        await storage.setItem("storage-key", newChunkedValue);
        expect(document.cookie).toEqual(`random-cookie=random;storage-key.0=${newChunkedValue.substring(0, MAX_CHUNK_SIZE)};storage-key.1=${newChunkedValue.substring(MAX_CHUNK_SIZE)}`);
    });
});
...
describe("createStorageFromOptions for createBrowserClient", () => {
    describe("storage with getAll, setAll", () => {
        ...
        it("should do chunk management with setAll (non-chunked => chunked case)", async () => {
            ...
            const chunkedValue = Array.from({ length: MAX_CHUNK_SIZE + 1 }, () => "x").join("");
        ...
        it("should do chunk management with setAll (less chunks => more chunks case)", async () => {
            ...
            const chunkedValue = Array.from({ length: 2 * MAX_CHUNK_SIZE + 1 }, () => "x").join("");
            await storage.setItem("storage-key", chunkedValue);
            expect(setAllCalls).toEqual([
                {
                    name: "storage-key.4",
...
describe("applyServerStorage", () => {
    it("should call setAll with the correct cookies for a variety of changes to the storage state", async () => {
        ...
        const newChunkedValue = Array.from({ length: MAX_CHUNK_SIZE + 1 }, () => "x").join("");
        await storage.setItem("storage-key", newChunkedValue);
