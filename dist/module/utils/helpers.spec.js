...
parseCookieHeader, serializeCookieHeader, } from "./helpers";
describe("helpers", () => {
    describe("parseCookieHeader", () => {
        it("should parse a Cookie header", () => {
            expect(parseCookieHeader("")).toMatchSnapshot();
            expect(parseCookieHeader(`a=b;c=${encodeURIComponent(" hello ")};e=f`)).toMatchSnapshot();
        });
    });
    describe("serializeCookieHeader", () => {
        it("should serialize a cookie to a Set-Cookie header", () => {
            expect(serializeCookieHeader("a", "", {
                path: "/",
                maxAge: 123,
                httpOnly: true,
                secure: true,
            })).toMatchSnapshot();
            expect(serializeCookieHeader("b", " weird value", { maxAge: 345 })).toMatchSnapshot();
        });
    });
});
